...
const get_messages = () => {
    const haiku_line_one = comp_one_arr[Math.floor(Math.random() * comp_one_arr.length)];
    const haiku_line_two = comp_two_arr[Math.floor(Math.random() * comp_two_arr.length)];
    const haiku_line_three = comp_three_arr[Math.floor(Math.random() * comp_three_arr.length)];
